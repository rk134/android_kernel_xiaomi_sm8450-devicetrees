				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l41->dsp.booted) {
	  ...
	  val = cpu_to_be32(val);
	  wm_adsp_write_ctl(&cs35l41->dsp, "CH_BAL", WMFW_ADSP2_XM, 0xcd, (void *)&val, sizeof(__be32));
	}
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (cs35l41->dsp.booted) {
	  wm_adsp_read_ctl(&cs35l41->dsp, "CH_BAL", WMFW_ADSP2_XM, 0xcd, (void *)&val, sizeof(__be32));
...
static int cs35l41_rcv_handset_switch_init(struct cs35l41_private *cs35l41)
{
	...
	cs35l41->enable_rcv_pin_control = 0;
	cs35l41->cs35l41_rcv_pinctrl = devm_pinctrl_get(cs35l41->dev);
	if (IS_ERR(cs35l41->cs35l41_rcv_pinctrl)) {
		if (PTR_ERR(cs35l41->cs35l41_rcv_pinctrl) == -EPROBE_DEFER) {
			dev_err(cs35l41->dev, "pinctrl not ready\n");
			ret = -EPROBE_DEFER;
			return ret;
		}
		dev_err(cs35l41->dev, "Target does not use pinctrl\n");
		cs35l41->cs35l41_rcv_pinctrl = NULL;
		ret = -EINVAL;
		return ret;
	}
	for (i = 0; i < ARRAY_SIZE(rcv_pctl_names); i++) {
		const char *n = rcv_pctl_names[i];
		struct pinctrl_state *state =
			pinctrl_lookup_state(cs35l41->cs35l41_rcv_pinctrl, n);
		if (IS_ERR(state)) {
			dev_err(cs35l41->dev, "cannot find '%s'\n", n);
			ret = -EINVAL;
			//goto exit;
			return ret;
		}
		dev_err(cs35l41->dev, "found pin control %s\n", n);
		cs35l41->pinctrl_state[i] = state;
		cs35l41->enable_rcv_pin_control = 1;
	}
	return ret;
}
#endif
